...
def create_model_tokenizer(name):
    ...
    model = AutoModelForCausalLM.from_pretrained(
        ...
        # gpt 2 not support flash attention2
    ...
    # /root/miniconda3/envs/llm/lib/python3.11/site-packages/trl/trainer/sft_trainer.py:397:
    # UserWarning: You passed a tokenizer with `padding_side` not equal to `right` to the SFTTrainer.
    # This might lead to some unexpected behaviour due to overflow issues when
    # training a model in half-precision.
...
def train():
    ...
    train_datasets, _ = create_sft_datasets(
        datasets, tokenizer, format_fun, seq_length)
