...
'''
run as 'python carpole-mcts-pytorch.py'
'''
...
print('In the ' + GAME_NAME + ' environment there are: ' +
      str(GAME_ACTIONS) + ' possible actions.')
print('In the ' + GAME_NAME +
      ' environment the observation is composed of: ' + str(GAME_OBS) + ' values.')
...
class Node:
    ...
    def create_child(self):
        ...
        for i in range(GAME_ACTIONS):
            ...
            copied_env = gym.make('CartPole-v1', render_mode='rgb_array')
            copied_env.reset()
            copied_env.np_random.bit_generator.state = deepcopy(
                self.game.np_random.bit_generator.state)
            games.append(copied_env)
            # render_mode = 'rgb_array'

        child = {}
        for action, game in zip(actions, games):
            observation, reward, done, dummy_1, dummy_2 = game.step(action)
    ...
    def rollout(self):
        ...
        # new_game = deepcopy(self.game)
        new_game = gym.make('CartPole-v1', render_mode='rgb_array')
        new_game.reset()
        new_game.np_random.bit_generator.state = deepcopy(
            self.game.np_random.bit_generator.state)
        ...
        while not done:
            ...
            observation, reward, done, dummy_1, dummy_2 = new_game.step(action)
            v = v + reward
            if done:
                new_game.reset()
                new_game.close()
