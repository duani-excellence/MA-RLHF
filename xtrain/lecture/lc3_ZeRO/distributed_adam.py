...
class DistributedAdam:
    def __init__(self, params, lr = 1e-3, beta1 = 0.90, beta2 = 0.999,  eps=1e-8, world_size = 1):
        super(DistributedAdam, self).__init__()
        self.eps = eps
        self.lr = lr
        self.beta1 = beta1
        self.beta2 = beta2
...
def run(rank, master_addr, master_port, world_size, backend='gloo'):
    ...
    model = ToyModel()
    loss_fn = nn.MSELoss()
    optimizer = DistributedAdam(model.parameters(), lr=0.0001, world_size = world_size)
