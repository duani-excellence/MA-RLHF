# python pipeline_parallel_dualpipe.py
'''
ref: https://github.com/deepseek-ai/DualPipe
官方 DualPipe 实现比较完整, 本代码实现简化版本
...
from zero_bubble_seperate_dx_dw import PipelineStage, WeightGradStore
from dualpipe_utils import run_backward, scatter, gather, ref_step, criterion, set_p2p_tensor_shapes, set_p2p_tensor_dtype, append_irecv, append_isend, TENSOR_DTYPE, TENSOR_SHAPES
...
class DualPipe(nn.Module):
    ...
    def _recv_forward(self, phase: int) -> None:
        ...
        tensors = append_irecv(self.comm_ops, self.prev_rank if phase == 0 else self.next_rank, self.group)
    ...
    def _send_forward(self, phase: int) -> None:
        ...
        append_isend(self.comm_ops, tensors, self.next_rank if phase == 0 else self.prev_rank, self.group)
    ...
    def _recv_backward(self, phase: int) -> None:
        ...
        tensors = append_irecv(self.comm_ops, self.next_rank if phase == 0 else self.prev_rank, self.group)
    ...
    def _send_backward(self, phase: int) -> None:
        ...
        append_isend(self.comm_ops, tensors, self.prev_rank if phase == 0 else self.next_rank, self.group)
    ...
    def step(
        ...
        *inputs: Optional[torch.Tensor],
        ...
        """
        # assert TENSOR_SHAPES is not None and TENSOR_DTYPE is not None, \
...
def run(rank, master_addr, master_port, world_size, backend='gloo'):
    ...
    #     dist.all_gather_into_tensor(p0all.unsqueeze(0), p0.grad)
    #     dist.all_gather_into_tensor(p1all.unsqueeze(0), p1.grad)
