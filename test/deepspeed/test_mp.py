...
pytestmark = pytest.mark.skipif(
    not required_torch_version(min_version=1.5, max_version=1.13),
...
def get_deepspeed_model(model):
    ...
    model, _, _, _ = deepspeed.initialize(
        model=model, mpu=mpu, model_parameters=model.parameters(), config=ds_config_dict
    )
    return model
...
class TestConfigurableMP(ConfigurableMP):
    ...
    def test_gpt2_basic(self, tmpdir, inputs):
        ...
        baseline = model(
            inputs[0].to(device_name), inputs[1].to(device_name), inputs[2].to(device_name)
        )
        ...
        model.load_checkpoint(
            tmpdir, tag=tag, load_optimizer_states=False, load_lr_scheduler_states=False
        )
...
class TestConfigurableResizeMP(ConfigurableMP):
    ...
    def test(self, baseline_mp2, inputs, class_tmpdir):
        ...
        with torch.no_grad():
            model.load_checkpoint(
                class_tmpdir, load_optimizer_states=False, load_lr_scheduler_states=False
            )
            device_name = get_accelerator().device_name()
            test = model(
                inputs[0].to(device_name), inputs[1].to(device_name), inputs[2].to(device_name)
